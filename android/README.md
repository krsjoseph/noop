...
An **offline WHOOP companion** for Android. Kineva connects directly to a WHOOP 4.0
...
(`/path/to/Kineva`, Swift). The protocol, framing, and BLE handshake are
...
```
android/
├── settings.gradle.kts          # rootProject "Kineva", includes :app
...
Root package: `com.noop` · application id: `com.aretetechnologies.kineva` (debug builds append `.debug`).

---

## Permissions & why

Kineva requests only what BLE needs, and deliberately omits `INTERNET`:
...
- [ ] Dark Kineva theme renders (surfaceBase `#060A08`, accent `#18C98B`); no white flash on launch.

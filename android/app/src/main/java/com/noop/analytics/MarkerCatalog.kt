 ...
 *     prompting the user to copy the range FROM THEIR OWN REPORT — Kineva never defines,
 *     computes, or asserts a normal range.
 *   - `higherIsBetter` is intentionally null for every entry: Kineva makes no value
...
data class MarkerDefinition(
    ...
    /** Direction hint — ALWAYS null (Kineva makes no value judgement). */
...
object MarkerCatalog {
    ...
    /** Neutral hint shown in the range field — the user copies their own report's range; Kineva ships none. */

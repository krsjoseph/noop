 ...
 *   1 — Derived on-device from raw by Kineva (computed recovery/strain/sleep from strap streams).
...
object MetricArbitrationPolicy {
    ...
    fun tier(metric: MetricKind, source: FusionSource): Int = when (metric) {
        ...
        MetricKind.STEPS ->
            ...
            when (source) {
                ...
                FusionSource.NOOP_COMPUTED -> 3  // Kineva step estimate from motion
                FusionSource.NUTRITION_CSV -> 3
                FusionSource.LOCAL_CACHE -> 3
            }

        MetricKind.SLEEP ->
            // The best STAGER wins: imported WHOOP stages > Kineva-computed stages > phone sleep buckets.
            when (source) {
                FusionSource.WHOOP_IMPORT -> 0
                FusionSource.NOOP_COMPUTED -> 1
     ...
     * sourceCandidates: imported WHOOP first, then Kineva-computed, then phone (Apple before Health

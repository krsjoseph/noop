...
object WhoopCsvImporter {
     ...
     * Kineva's "Effort" score lives on 0–100 (StrainScorer.maxStrain = 100). Rescale an imported Day
        ...
        for (row in table.rows) {
            ...
            out.add(
                DailyMetric(
                    ...
                    avgHrv = avgHrv,
                    recovery = recovery,
                    // Rescale WHOOP's 0–21 Day Strain onto Kineva's 0–100 Effort axis (see
    ...
    private fun parseWorkouts(table: CsvTable, deviceId: String): List<WorkoutRow> {
        ...
        for (row in table.rows) {
            ...
            // Workout strain is also WHOOP's 0–21 scale → rescale onto Kineva's 0–100 Effort axis so

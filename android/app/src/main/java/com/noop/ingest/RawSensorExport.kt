...
/**
 * EXPERIMENTAL diagnostic: dump the decoded per-sample sensor streams Kineva already stores to ONE
    ...
    suspend fun export(context: Context, repo: WhoopRepository, deviceId: String = "my-whoop") {
        runCatching {
            ...
            val counts = file.bufferedWriter().use { w ->
                w.append("# Kineva raw sensor export · last 24h · long-format CSV\n")
            ...
            val send = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_SUBJECT, "Kineva raw sensor export")

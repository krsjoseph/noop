...
/**
 * Serializes Kineva's own cached rows back into WHOOP's 4-CSV export shape so Kineva's OWN importer
 ...
 * literal "UTC+00:00" timezone column: Kineva stores epoch seconds and tz-less day strings, so UTC is
...
object WhoopCsvExporter {
    ...
    /**
     * Stage minutes recovered from any persisted stagesJSON shape Kineva has ever written:
        ...
        for (d in daily.sortedBy { it.day }) {
            ...
            sb.append(
                listOf(
                    ...
                    // is WHOOP-format and a Kineva→Kineva round-trip is lossless (import scales back ×100/21).
     ...
     * Kineva→Kineva round-trip is unchanged (the importer keys on sleep_onset, not Cycle start time).
     */
        ...
        for (s in sessions.sortedBy { it.startTs }) {
            ...
            sb.append(
                listOf(
                    ...
                    // Kineva never stores a nap flag — everything exports as a main sleep so the

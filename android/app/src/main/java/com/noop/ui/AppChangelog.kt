    ...
    val releases: List<Release> = listOf(
        ...
        Release(
            ...
            items = listOf(
                ...
                "**Storage clean-up (iPhone).** A failed or retried Apple Health import could strand a multi-gigabyte copy the Storage screen never saw; Kineva now reclaims those leftovers automatically. (No Android-facing change — this was an iPhone import path.) Thanks @exzanimo (#590).",
            ),
        ),
        Release(
            version = "6.2.1",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "**Fixed: WHOOP 4.0 pairing could get stuck in a loop on some OnePlus phones.** Their Bluetooth fires the connection setup twice in a row, which wedged the secure handshake — Kineva now ignores the duplicate and gives the link a moment to settle, so pairing completes. (#50)",
            )),
        Release(
            version = "5.2.2",
        ...
        Release(
            ...
            items = listOf(
                ...
                "**Smarter reconnect when your strap's out of range** — Kineva backs off gradually instead of rescanning on a fixed timer (easier on battery), and reconnects instantly the moment you tap Connect. Thanks to **ryanbr** for the contribution.",
            )),
        Release(
            version = "5.1.2",
        ...
        Release(
            ...
            items = listOf(
                ...
                "**Trends report adds Workouts and Stress.** The exportable Trends report now leads with a **Workouts** row (your activity count over the range) and a **Stress** row (Kineva's 0–3 daily autonomic-load trend), each with its own averages and a measured-vs-computed note. (#457)",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "**Kineva now has a full Light theme, and you can switch any time.** Settings → Appearance lets you pick **System** (follow your phone), **Light**, or **Dark**. The new Light look is \"warm paper & gold\" — a soft warm-white canvas with crisp navy-ink text and the signature gold deepened so it stays legible on white. Every surface was re-done for it, not just inverted: the ring gauges, frosted cards (now lifted with a soft shadow instead of a glow), charts, the scenic hero, the home-screen widget and even the status bar all adapt. Dark stays exactly as it was. Same data, same layout — your choice of finish.",
            ),
        ),
        Release(
            version = "4.2.13",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "**Fixed Kineva crashing — or refusing to open at all — whenever Bluetooth was on.** This hit some phones hard, notably WHOOP 5.0 / MG on Android 16. When Bluetooth came on, Kineva's background service reconnected to your saved strap and logged the first frame it received; a bug in the privacy log-redaction code (it masks Bluetooth addresses) threw an error on that line and crashed the **entire app — even while it was closed**, and earlier builds had it too, so downgrading didn't help. Two fixes: the redaction bug is gone, and the **logging path is now hardened so a diagnostic line can never crash the app again** (with a regression test). Your data and history were never at risk. Huge thanks to @frazzle28 and @pawan0305 for the reports and the crash trace (#453).",
            ),
        ),
        Release(
            version = "4.2.11",
            ...
            date = "June 2026",
            items = listOf(
                "**Fixed a crash that stopped Polar H10 and other standard Bluetooth heart-rate straps from connecting.** When Kineva tried to activate a generic HR strap, an internal logging bug threw an error the instant it wrote the strap's Bluetooth address into the log — and that error quietly aborted the connection, so the strap paired but never streamed live data. Generic HR straps now connect and stream as intended. (WHOOP straps were never affected — they don't log a raw address.) Thanks @pilleuspulcher-blip for the strap log that pinned it down (#421).",
            ),
        ),
        Release(
            version = "4.2.10",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "A gentle move reminder, if you want one. Turn it on in Settings → Automations and Kineva buzzes your strap after you've been sitting still too long (your threshold, default 45 min), within hours you choose (default 9–5), with a cooldown you set. Off by default, runs from the motion already on your strap, and respects quiet hours and only-when-worn. Thanks @cbarrado (#419).",
            ),
        ),
        Release(
            version = "4.2.0",
            ...
            items = listOf(
                ...
                "Last night syncs sooner: Kineva keeps a deep backlog draining while you're connected instead of waiting 15 minutes between bursts, plus a Sync now button to backfill on demand. Thanks @idkwargwanbear (#364).",
        ...
        Release(
            ...
            items = listOf(
                ...
                "Honest Apple Health guidance on free sideloads (iPhone): if you installed Kineva with a free Apple ID (AltStore / Sideloadly), the build can't be granted Apple Health access — so instead of pointing you to a Settings screen Kineva can never appear in, it now tells you straight and routes you to the file-import / Shortcuts path. Thanks @exzanimo (#348).",
        ...
        Release(
            ...
            items = listOf(
                ...
                "Sleep (iPhone/Mac): you can now hand-correct a night's bed/wake times with a pencil on the Sleep tab — Kineva re-stages from the raw sensor data and the correction survives the next strap sync. (Android already has bed/wake editing; durable-edit parity is tracked.) Thanks @claypilat (#395).",
            ),
        ),
        Release(
            version = "3.4.0",
        ...
        Release(
            ...
            date = "June 2026",
            items = listOf(
                "New: a **smart wake alarm** — set a wake window and Kineva wakes you on a lighter sleep phase inside it, with a guaranteed alarm at the end of the window. The guaranteed wake is a real OS alarm that fires even if Bluetooth drops or the app is closed. Thanks @subscriptiondestroyer (#207).",
        ...
        Release(
            ...
            items = listOf(
                ...
                "New: if Kineva ever crashes, the details are now saved into the strap log you share — so a crash that only happens on your device can actually be diagnosed (#33).",
        ...
        Release(
            ...
            items = listOf(
                ...
                "Fixed (iPhone): if a Kineva backup from another platform had been restored (e.g. an Android backup onto an iPhone), the app could get permanently stuck on “store not ready”. Kineva now recovers automatically on the next launch, and declines such a backup at import time with a clear explanation. To move history across platforms, use the WHOOP-format CSV export instead. Thanks @NoahMcE. (#222)",
            ),
        ),
        Release(
            version = "2.8.4",

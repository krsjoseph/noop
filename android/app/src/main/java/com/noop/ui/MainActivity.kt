...
object NoopPrefs {
    ...
    /** "Continuous HRV capture" — when on (AND background connection is on), Kineva holds the dense
    ...
    /** "Broadcast heart rate" — when on, Kineva acts as a standard BLE Heart Rate peripheral (0x180D /
    ...
    /** Whether Kineva keeps the dense realtime HR stream armed 24/7 for continuous HRV capture. Default
    ...
    /** Whether Kineva re-broadcasts its live HR as a standard BLE Heart Rate peripheral. Default OFF. */
    ...
    /** Health Connect periodic auto-sync (Samsung Health → Health Connect → Kineva). Default OFF.
    ...
    /** Health Connect writeback (Kineva's computed metrics → HC, for other apps). Default OFF. */
    ...
    /** "Auto-detect workouts" (MVP, opt-in, on-device, NON-DESTRUCTIVE). When ON, Kineva scans the last
    ...
    /** The last strap we bonded to (address + model), persisted so Kineva can reconnect to it directly on

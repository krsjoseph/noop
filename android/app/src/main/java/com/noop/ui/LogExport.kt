...
object LogExport {
    ...
    fun writeScheduledExport(context: Context, logText: String, nowMs: Long = System.currentTimeMillis()): List<File> =
        runCatching {
            ...
            val header = buildString {
                appendLine("Kineva strap log (scheduled debug export)")
    ...
    private fun writeCaptureFile(context: Context): File? {
        ...
        val header = buildString {
            appendLine("# Kineva 5/MG raw backfill capture (JSONL; one frame per line)")
    ...
    fun shareStrapLog(context: Context, logText: String) {
        runCatching {
            ...
            val send = Intent(Intent.ACTION_SEND).apply {
                type = "text/plain"
                putExtra(Intent.EXTRA_STREAM, fileUri(context, file))
                putExtra(Intent.EXTRA_SUBJECT, "Kineva strap log")
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
    ...
    fun shareWhoop5Capture(context: Context, whoop5Connected: Boolean) {
        runCatching {
            ...
            val send = Intent(Intent.ACTION_SEND).apply {
                ...
                putExtra(Intent.EXTRA_SUBJECT, "Kineva 5/MG protocol capture")
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }

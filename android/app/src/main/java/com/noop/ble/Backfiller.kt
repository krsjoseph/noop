...
class Backfiller(
     ...
     * (#150): Kineva logged FAILURES (decoded-to-0) but never SUCCESSES, so a strap log couldn't tell a
    ...
    private suspend fun finishChunk(unix: Long, trim: Long, endFrame: ByteArray) {
        ...
        if (frames.isNotEmpty()) {
            ...
            // is visible in a shared log (the strap clock is genuinely bad — this is Kineva being robust).
    ...
    companion object {
         ...
         * strap's clock, not a Kineva bug) and gives the fix (charge + reconnect re-syncs the RTC). Byte-
        ...
        fun futureRtcLine(endUnix: Long, wallNowUnix: Long): String {
            ...
            return "Backfill: the strap reported a record dated about $aheadDays day(s) in the FUTURE - " +
                "its clock (RTC) is corrupt, not a Kineva problem. Those records can't be filed onto the " +

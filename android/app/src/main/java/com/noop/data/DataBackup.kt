        ...
        when (backupOriginOf(backupTables)) {
            BackupOrigin.MAC ->
                return rejectForeign(
                    tempSqlite,
                    "This isn't a Kineva backup from this app. It looks like a backup from the Mac or " +
                        "iOS Kineva app (it carries that platform's migration bookkeeping). Restoring it here " +
            ...
            BackupOrigin.UNKNOWN ->
                if (holdsData(backupTables)) {
                    return rejectForeign(
                        tempSqlite,
                        "This isn't a Kineva backup from this app. It's missing the database bookkeeping a " +
                            "Kineva backup carries (it looks like another app's database). Restoring it would " +
                            "strand your store.",
                    )
                }

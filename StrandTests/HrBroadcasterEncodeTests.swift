...
/// Pins the pure 0x2A37 Heart Rate Measurement *encoder* used by `HrBroadcaster` when Kineva re-broadcasts
...
final class HrBroadcasterEncodeTests: XCTestCase {
    ...
    func testRoundTripThroughParser() {
        for bpm in [37, 55, 72, 100, 180, 254, 255, 256, 300] {
            ...
            XCTAssertEqual(parsed?.rr.count, 0, "Kineva broadcasts a plain HR with no R-R")
        }
    }
}

...
ZIP="${2:-$HOME/Downloads/Kineva-v${VER}-macos.zip}"
...
cask "noop" do
  ...
  url "https://github.com/${ORG}/${REPO}/releases/download/v#{version}/Kineva-v#{version}-macos.zip"
  name "Kineva"
  ...
  caveats "Kineva ships anonymously and is unsigned (no Apple Developer ID), so on first launch macOS Gatekeeper will block it. On macOS 15 Sequoia and later: try to open Kineva once, then go to System Settings > Privacy & Security, scroll down, and click 'Open Anyway' next to Kineva. (On macOS 14 and earlier you can right-click Kineva in /Applications and choose Open.) Update later with: brew upgrade --cask noop."
end
EOF

...
//     placeholder prompting the user to copy the range FROM THEIR OWN REPORT — Kineva
//     never defines, computes, or asserts a normal range.
//   - `higherIsBetter` is intentionally `nil` for every entry: Kineva makes no value
...
public struct MarkerDefinition: Sendable, Equatable, Codable {
    ...
    /// Direction hint — ALWAYS `nil` (Kineva makes no value judgement). Present only as

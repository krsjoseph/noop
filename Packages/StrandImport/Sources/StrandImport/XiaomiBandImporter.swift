...
/// the daily-rollup tables and the per-session `sleep` table into Kineva's shapes.
///
/// This is **parse-only**: it never touches Kineva's own store, only the foreign file
...
public struct XiaomiBandImporter {
    ...
    // Day-rollup tables, keyed by Kineva day. (`goal_day` is intentionally skipped —

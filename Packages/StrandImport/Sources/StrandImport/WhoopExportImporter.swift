...
public struct WhoopExportImporter {
    ...
    /// WHOOP reports "Day Strain" on its own 0–21 logarithmic scale. Kineva's "Effort" score lives on a
    ...
    /// Rescale an imported WHOOP Day Strain (0–21) onto Kineva's 0–100 Effort axis. `nil` passes through.
    ...
    /// Inverse: convert Kineva's internal 0–100 Effort back onto WHOOP's 0–21 Day Strain scale for a
    /// WHOOP-format CSV export. Keeps the CSV genuinely WHOOP-compatible AND makes a Kineva export →
    /// Kineva import round-trip lossless (export ÷scale, then import ×scale restores the value).

...
/// literal "UTC+00:00" timezone column: Kineva stores epoch seconds and tz-less day strings, so UTC
    ...
    public static func cyclesCSV(days: [DailyMetric],
        ...
        for d in days.sorted(by: { $0.day < $1.day }) {
            ...
            let cols: [String] = [
                ...
                // WHOOP-format and a Kineva→Kineva round-trip is lossless (importer scales it back up).
    ...
    public static func sleepsCSV(_ sessions: [CachedSleepSession],
        ...
        for s in sessions.sorted(by: { $0.startTs < $1.startTs }) {
            ...
            let cols: [String] = [
                ...
                // Kineva never stores a nap flag — everything exports as a main sleep so the importer

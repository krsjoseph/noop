...
public enum DataSourceKind: String, Sendable, Codable, Equatable, CaseIterable {
    ...
    /// fully offline: Kineva reads the file the user already owns.
    case xiaomiBand
...
/// The canonical sleep stages Kineva recognises from the Mi Fitness `sleep` table's
...
/// One sleep session imported from a wearable export. Durations are MINUTES (as Kineva's
...
/// as Kineva's Charge. Kineva recomputes its own scores downstream from the raw inputs (RHR / HRV /
...
public struct WearableDailyRow: Sendable, Equatable {
    ...
    // The brand's OWN reference scores — stored under reference keys, never Kineva Charge/Effort/Rest.

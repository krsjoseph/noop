...
// ("on-device core"). The recovery ring uniquely carries a micro "Kineva" wordmark
// above the number (letter-spacing ≈ .34em, tertiary) so the lock-up reads as the
// "O" in Kineva. The arc geometry, gradient stroke, track and centre number live in
...
public struct RecoveryRing: View {
    ...
    /// Whether to draw the micro "Kineva" wordmark above the number. Turn it OFF for compact rings
    ...
    public var body: some View {
        ZStack {
            ...
            // open-ring + core-dot lock-up reads), then the micro "Kineva" wordmark
    ...
    /// Micro "Kineva" wordmark above the number — the recovery ring carries the
    /// lock-up so its centre reads as the "O" in Kineva. ALL-CAPS, tertiary,
    ...
    /// is hidden whenever a number is shown — leaving a clean ring + number + micro-Kineva wordmark.

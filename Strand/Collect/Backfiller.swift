...
final class Backfiller {
    ...
    /// only fires for layouts Kineva can't decode), this surfaces the layout on a HEALTHY sync too, so a
    ...
    /// strap's clock, not a Kineva bug) and gives the fix (charge + reconnect re-syncs the RTC). Byte-identical
    ...
    nonisolated static func futureRtcLine(endUnix: Int, wallNowUnix: Int) -> String {
        ...
        return "Backfill: the strap reported a record dated about \(aheadDays) day(s) in the FUTURE - its clock (RTC) is corrupt, not a Kineva problem. Those records can't be filed onto the right day. Fully charge the strap to 100% and reconnect so it re-syncs its clock; if it persists, forget and re-pair the strap."
    }
    ...
    private func finishChunk(unix: UInt32, trim: UInt32, endFrame: [UInt8]) async {
        ...
        // not a Kineva bug) is named and the fix (charge + reconnect re-syncs the RTC) is given. Observability
        ...
        if !frames.isEmpty {
            ...
            // unmapped-version path below only fires for layouts Kineva can't decode, so a normal log
            ...
            for p in parsed {
                ...
                log?("Historical records use firmware layout v\(v), which Kineva doesn't decode yet — no motion data, so sleep can't be computed from the strap. Please report this (issue #30).")
            }
            ...
            // is the strap's clock, not a Kineva decode bug. Observability only; the gate already did the work.

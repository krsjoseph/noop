...
/// Re-broadcasts Kineva's LIVE heart rate back OUT as a standard Bluetooth Heart Rate peripheral, so a
/// gym treadmill, Zwift, Peloton, a bike computer, or any fitness app can read the WHOOP HR that Kineva
...
/// SIG-spec flags + bpm encoding whenever Kineva has a fresh live HR sample.
///
/// OFFLINE, OPT-IN, ADDITIVE
/// -------------------------
/// This is LOCAL Bluetooth only — nothing leaves the device to any cloud or server. It just re-shares
/// the strap's HR to nearby gym kit over a standard BLE profile, which fits Kineva's offline ethos. It is
...
public final class HrBroadcaster: NSObject, ObservableObject {
    ...
    /// its own HR via a firmware device-config; THIS one makes the Kineva *app* act as the HR peripheral.
    ...
    /// SAME exportable strap log the WHOOP path uses, so a tester whose gym kit can't see Kineva has a
    ...
    /// We never set bit3 (Energy Expended) or bit4 (R-R) — Kineva broadcasts a plain instantaneous HR. The
    ...
    private func beginAdvertising() {
        ...
        if !manager.isAdvertising {
            manager.startAdvertising([
                ...
                CBAdvertisementDataLocalNameKey: "Kineva HR",
            ])
...
extension HrBroadcaster: @preconcurrency CBPeripheralManagerDelegate {
    public func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager) {
        ...
        case .unauthorized:
            advertising = false
            statusNote = "Kineva needs Bluetooth permission to broadcast your heart rate."

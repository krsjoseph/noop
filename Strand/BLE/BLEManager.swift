...
/// (#80). On a flaky radio (2016 Mac / OpenCore) the link dies the *instant* Kineva arms that
...
/// connect and relies on the independent low-bandwidth 0x2A37 standard HR profile, which Kineva already
...
struct BondRefusalGiveUp {
    ...
    static func pausedHint() -> String {
        "Kineva stopped retrying because your strap keeps refusing to pair. It is likely still held by the official WHOOP app, or your phone is holding an old pairing. Close the WHOOP app, put the strap in pairing mode (tap until the LEDs flash blue), and if it is listed in your Bluetooth settings choose Forget This Device. Then tap Connect to try again."
    }
...
public final class BLEManager: NSObject, ObservableObject {
    ...
    public func connect(model: WhoopModel = .persisted) {
        ...
        if let p = existing.first(where: { isPreferredPeripheral($0) }) {
            ...
            // platforms an LE link is shared system-wide, so a strap held by the WHOOP app, a prior Kineva
            // session, or the OS itself reads .connected while Kineva has NO session of its own yet. The old
    ...
    /// row alone left the strap connected — Kineva kept re-grabbing it (the 3s disconnect→reconnect timer, the
    ...
    /// restoration references that point at this strap so Kineva lets go for good — until the user deliberately
    ...
    public func enableWhoop5DeepData() {
        ...
        guard state.connected, state.encryptedBond else {
            ...
            log("Deep-data: needs the full encrypted bond, not the live-HR-only link. Close the official WHOOP app, put the strap in pairing mode, and bond it to Kineva first — ignored."); return
        }
    ...
    /// the fixed time — Kineva has no light-sleep early-wake layer.
    ///
...
extension BLEManager: @preconcurrency CBCentralManagerDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        // Kineva itself works fine on the new firmware once the stale bond is cleared. (5/MG firmware reset, 2026-06)
        ...
        if let cbErr = error as? CBError, cbErr.code == .peerRemovedPairingInformation {
            state.reconnectGuide = """
            Your strap's Bluetooth pairing was reset — usually by a WHOOP firmware update, or the official WHOOP app reconnecting. Kineva works fine on the new firmware; you just need to re-pair:
        ...
        if let error = error {
            ...
            if selectedModel.deviceFamily == .whoop5, !didBond, insufficient {
                ...
                if bondRefusalStreak >= 2 {
                    state.pairingHint = "Kineva can see your strap but it's refusing to pair — it's likely still bonded to the official WHOOP app, or your phone is holding an old pairing. To fix it: (1) fully close the WHOOP app, (2) on a 5.0/MG, tap the band repeatedly until the LEDs flash blue (pairing mode), (3) if your strap is listed under iPhone Settings → Bluetooth, tap it and choose Forget This Device, then reconnect in Kineva."

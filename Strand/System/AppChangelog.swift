...
enum AppChangelog {
    ...
    static let currentVersion = "7.5.0"
    ...
    static let releases: [Release] = [
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**The dashboard stays responsive while your strap syncs (iPhone and Mac).** If you've imported a large history (a WHOOP export plus Apple Health), the Today screen could freeze for several seconds when you opened it or returned to the tab, and stutter when you scrolled, all while the strap was offloading its history in the background. Kineva now paints the day's data instantly and runs the heavy history reads without fighting the sync, so it stays smooth. (#755)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**New: \"~X days left\" on your strap battery.** Kineva watches how fast the band is discharging and tells you roughly how many days are left, right on the Today battery badge. All on-device, nothing logged.",
        ...
        Release(
            ...
            items: [
                ...
                "**A wildly out-of-range imported HRV no longer shows a nonsense headline.** An imported HRV value that was far outside any believable range could drive a silly \"way over baseline\" headline. Kineva now ignores the impossible value instead of building a verdict on it.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**A whole new look.** Kineva has been redesigned from the ground up — flat, clean colour rings, a day-cycle scene that moves with your day, and a Today screen you can customise to show what matters to you. The same fresh look lands on iPhone, Mac and Android together.",
                ...
                "**New: Automatic workout detection.** Opt in and Kineva spots a likely workout from your heart rate and motion and offers it for a one-tap add, so a session you forgot to start doesn't go unrecorded. Nothing is logged without you confirming it. Off by default.",
        ...
        Release(
            ...
            items: [
                ...
                "**Storage clean-up that actually reclaims it.** A failed or retried Apple Health import could strand a multi-gigabyte unzipped copy that the Storage screen never saw, so it kept showing a huge footprint. Kineva now recognises and sweeps those leftovers automatically on launch, and the Clean up button reclaims them too. Thanks @exzanimo (#590).",
        ...
        Release(
            ...
            items: [
                ...
                "**Tighter sleep dates.** A WHOOP with a wandering clock could re-send records stamped with wrong dates and scramble which night was which. Kineva now checks each record against the strap's own data range and drops the impossible ones (#547).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Sleep got smarter and more honest.** A night split by a wake-up is now counted in full instead of just one fragment. A bad-clock strap can no longer pass off a 12-hour block as one night. A still morning right after you wake is no longer mistaken for a second sleep. And when the deep/REM split can't be trusted on a quiet night, Kineva says so instead of guessing. Your own hand-edits to a night also win over an imported value now.",
                "**Naps, spotted on your device.** Opt in and Kineva notices a likely nap from your motion and offers it for a one-tap add. Nothing is logged automatically, and it never touches your real sleep scores. Thanks @cbarrado.",
                "**WHOOP 4.0 sleep on older firmware.** Straps on an older offload layout that used to bank nothing now hand over the motion Kineva needs to stage sleep. Thanks airtonzanon for the captures.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Fixed: a WHOOP with a bad internal clock could scramble your dashboard.** If your strap's clock or flash got into a bad state, it could hand Kineva records stamped with wrong dates, sometimes years off, sometimes in the future. Kineva now sanity-checks every record's timestamp as it comes in and drops anything implausible, so a misbehaving strap can no longer make the same sleep repeat across days or show a future date as your last night. If your data already got scrambled, updating cleans it up automatically and re-scores once. Thanks to pikapik487 for the detailed logs that pinned this down.",
            ]),
        Release(
            version: "6.0.2",
        ...
        Release(
            version: "6.0.0",
            title: "Kineva grows up: it's not just for WHOOP anymore",
            date: "June 2026",
            items: [
                "**Your WHOOP is no longer the only thing that works.** Kineva now reads standard Bluetooth chest straps and arm bands (like the Polar H10) for live heart rate and HRV, connects to gym machines over the standard FTMS profile (treadmills, bikes, rowers, cross-trainers), and reads standard running and cycling sensors for live speed, cadence and power during a workout. Your WHOOP support is exactly as it was.",
                "**Bring your history with you, fully offline.** Import your own data export from Oura, Fitbit or Garmin and Kineva pulls in sleep, resting heart rate, HRV and steps wherever the file has them. It never talks to their cloud, and their own readiness or sleep scores stay reference only. Your Kineva scores are recomputed from the raw signals, never copied. GPX, TCX and FIT workout files import too.",
                "**Broadcast your heart rate out.** Turn on Broadcast in Data Sources and Kineva re-shares your strap's heart rate as a standard Bluetooth HR sensor, so a treadmill, Zwift or Peloton can read it. Local Bluetooth only, nothing leaves your device. Off by default.",
        ...
        Release(
            ...
            items: [
                ...
                "**No more \"New data added\" spam.** The Updates inbox used to repeat that every time Kineva re-scored your recent days in the background, even on an old import with nothing new. Now it tells you once, only when a genuinely newer day lands. (#521)",
                ...
                "**New: a daily auto-export of your strap log (iPhone & Mac).** Turn it on under **Settings → Diagnostics**, pick a time, and Kineva saves a timestamped copy once a day, so a log is waiting for a bug report without you remembering to grab it. Off by default, stays on your device. On Mac it runs while Kineva is open; on iPhone it fires when iOS next wakes the app near your time, not to the exact minute. Android already had this. (#510)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Kineva is back on GitHub** — and so is **Check for updates**. The in-app update check and the **Settings → About** \"project home\" link now point at github.com/NoopApp/noop again, where releases live (noop.fans stays as a mirror). It's still on-device and only runs when you tap — nothing about you is ever sent.",
            ]),
        Release(
            version: "5.2.5",
            ...
            date: "June 2026",
            items: [
                "**Fixed: removing a WHOOP from Devices now actually releases it.** Before, Kineva kept reconnecting to a removed strap and held it connected — so a 5/MG could never enter pairing mode, which blocked re-pairing a strap that got stuck. Remove now stops reconnecting, drops the link and frees the strap. (iPhone & Mac.) (#78)",
                "**Clearer help when a 5/MG won't bond.** If the strap keeps refusing the secure pairing, Kineva now tells you exactly how to free and re-pair it (close the WHOOP app, pairing mode, forget in Bluetooth) rather than a misleading \"transient\" message. (#78)",
            ]),
        Release(
            version: "5.2.4",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Fixed (WHOOP 5/MG): opening \"Add a WHOOP\" could drop a working strap and get stuck on \"connecting\".** If your 5/MG was connected and streaming, presenting the scan tore down the live connection — and the strap could then loop on \"connecting\" instead of re-bonding (with haptics going quiet). Kineva now keeps a live same-family connection while it scans for nearby straps. (iPhone & Mac.) (#74)",
                "**Clearer guidance when a reconnect briefly hiccups.** If a strap Kineva *just* bonded to momentarily refuses on a reconnect, it no longer wrongly tells you it's \"still paired to the WHOOP app\" — it recovers quietly instead. (#74)",
            ]),
        Release(
            version: "5.2.2",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Fixed (WHOOP 5/MG): pairing could get stuck and the buzz go silent.** If your strap had been re-paired or reset, Kineva could latch onto an old Bluetooth identity, fail to finish the secure bond and loop forever — which also stopped haptics. Kineva now notices a strap that *is* bonding fine and switches to it. (iPhone, Mac & Android.)",
                "**Fixed (WHOOP 4.0 on some Androids): stuck on \"finishing the secure handshake\".** On phones whose Bluetooth double-fires the connection setup (seen on OnePlus), pairing could wedge with no way out — Kineva now bounces and retries automatically instead of hanging.",
                ...
                "**Smarter reconnect when your strap's out of range** — Kineva backs off gradually instead of rescanning on a fixed timer (easier on battery), and reconnects instantly the moment you tap Connect. Thanks to **ryanbr** for the contribution. (Android — matches iPhone & Mac.)",
            ]),
        Release(
            version: "5.1.2",
        ...
        Release(
            ...
            items: [
                ...
                "**What Moves You.** A ranked, lag-aware read of what actually moves *your* recovery — from your own journal and outcomes, not population averages. Log alcohol or late caffeine with an amount and Kineva fits a personal dose-response curve, then in the evening tells you what one more drink tends to cost tomorrow's Charge. Open **What Moves You** (the wand in the sidebar / Insights).",
                ...
                "**Your Data, Fused.** If you wear more than one band, Kineva now shows one honest record — best source wins per metric, with the source named on every number and conflicts flagged, never silently averaged. Open **Your Data, Fused** from **Health** or Data Sources. A single WHOOP just shows a clean plain record.",
                "**Lab Book — your own private logbook.** Type in your bloods, blood pressure, scan values or doctor's-visit notes (or import a CSV), see each marker's trend, and line a marker up against a wearable signal with **Compare with a signal**. It's a notebook, not a medical service — Kineva stores and lines up the numbers *you* enter, never tests, reads or diagnoses them, and it all stays on your device. Open **Health → Lab Book**.",
        ...
        Release(
            ...
            items: [
                ...
                "**Apple Health body composition** — Kineva now reads your weight, body-fat %, lean mass and BMI from Apple Health on iPhone. (Thanks @h3ld3r — #20.)",
            ]),
        Release(
            version: "4.6.2",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Naps are now editable — and stay their own thing.** You can edit a detected nap's start and end times (Kineva re-stages it from your raw data and the correction sticks through future syncs), and manually add a nap the strap missed, right from the Sleep screen. Naps are always tracked as separate sessions from your main sleep, so the awake time between them is never mislabelled as light sleep. (#508)",
                "**Trends report adds Workouts and Stress.** The exportable Trends report now leads with a **Workouts** row (your activity count over the range) and a **Stress** row (Kineva's 0–3 daily autonomic-load trend), each with its own averages and a measured-vs-computed note, alongside recovery, sleep, HRV and the rest. (#457)",
        ...
        Release(
            ...
            items: [
                ...
                "**WHOOP 5/MG: steps are accurate now.** The strap's step counter is a *running total*, not a per-reading count — adding it up the old way could over-report steps many times over. Kineva now reads the full counter and adds only the real increases, so your daily step number is sane. It also reads a simple still / walking / running activity signal from the same data, with no cloud. *(Thanks j0b-dev for the analysis — #276 / #316.)*",
            ]),
        Release(
            version: "4.5.2",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "A quick refinement to yesterday's off-wrist sleep fix. Kineva now only discards a sleep block when **most of it** (half or more) is off-wrist, rather than dropping it for any off-wrist gap at all. So a real night where you take the strap off shortly after waking is kept in full, while a strap left sitting still on a desk all day is still correctly ignored. *(Thanks to community contributor j0b-dev for the sharper approach.)*",
            ]),
        Release(
            version: "4.5.0",
            ...
            date: "June 2026",
            items: [
                "**More of your WHOOP 5/MG history now syncs.** Some nights were stored by the strap in newer record layouts (internally \"v20/v21\") that Kineva didn't recognise yet, so they were skipped and showed up as empty. Those now decode — so more of your 5/MG history comes through. We also pull richer detail from the existing records (higher-precision heart rate, step cadence, an extra skin-temperature channel) and corrected the skin-temperature scale so worn readings land where they should. *(Thanks to community contributor j0b-dev for the captured-frame analysis behind this.)*",
                "**Sleep: no more daytime false sleep.** Time with the strap off your wrist — on the charger, or sat at a desk — could occasionally be logged as sleep. Kineva now spots those gaps (a long stretch with no real heart-rate signal, or an explicit off-wrist marker) and won't count them as sleep, day or night.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**The shareable Trends report now spells out where each number comes from.** A new \"How to read this\" legend flags HRV, Resting HR and Sleep as *measured* from the strap, and makes clear that **Recovery and Strain are Kineva's own on-device scores, not clinical measures** — so it's safe to hand the PDF to a doctor or coach without your scores being mistaken for lab values. Thanks @subscriptiondestroyer (#457).",
            ]),
        Release(
            version: "4.2.4",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**A gentle move reminder, if you want one.** Turn it on in Settings → Automations and Kineva will buzz your strap after you've been sitting still too long (your threshold, default 45 min), within hours you choose (default 9–5), with a re-nudge cooldown you set. It's **off by default**, runs entirely from the motion already on your strap, and respects your quiet hours and only-when-worn settings. Thanks @cbarrado (#419).",
            ]),
        Release(
            version: "4.2.0",
        ...
        Release(
            ...
            items: [
                ...
                "**Generic heart-rate straps now actually connect.** A Polar / Wahoo / Coospo strap you made active was being *discovered* but never *connected to* — so it sat there with no live data. Fixed: Kineva now connects straight to your selected strap. Thanks @pilleuspulcher-blip (#421).",
        ...
        Release(
            ...
            items: [
                ...
                "**Optional: see your estimated VO₂max.** Add your waist measurement in Settings and Kineva will also show an estimated VO₂max alongside your Fitness Age. (Your Fitness Age itself never needs it.)",
                "**Honest by design.** Every new number carries a ± band and a plain “this is a wellness estimate, not a clinical age” line. These build over a week or two of wear and sharpen as Kineva learns your baseline.",
            ]),
        Release(
            version: "3.9.1",
            ...
            items: [
                ...
                "**Android: GPS workouts write back to Health Connect.** Workouts you track in Kineva weren't being saved to Health Connect — we'd never asked for the exercise-write permission, so the system quietly dropped them. Fixed; you'll be asked once to allow exercise + distance. Thanks @andreasc1 (#412).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**Manage several WHOOP straps.** Got more than one WHOOP — a couple of 4.0s, a 5.0, or a mix? Kineva now tells them apart and lets you **pair, switch, rename and remove** each one from the **Devices** screen. Only one strap is ever active at a time, and your history is never mixed between devices.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**A new Devices screen.** Kineva can now read more than just a WHOOP. Pair a **standard Bluetooth heart-rate strap** — Polar, Wahoo, Coospo, a Garmin HRM, or the Amazfit Helio's heart-rate broadcast — for **live heart rate + HRV**. Manage everything under **Devices**: see what's paired, switch which strap is active, rename or remove one.",
                "**WHOOP stays the primary, fully-supported band.** Other straps are an early, opt-in addition — they stream live HR + HRV, but not WHOOP's deeper sleep, recovery and strain. Only one strap is ever active at a time, and Kineva never mixes data from two devices.",
        ...
        Release(
            ...
            items: [
                ...
                "**Honest Apple Health guidance on free sideloads:** if you installed Kineva with a free Apple ID (AltStore / Sideloadly), the build can't be granted Apple Health access at all — so instead of pointing you to a Settings screen Kineva can never appear in, it now tells you straight and routes you to the file-import / Shortcuts path. Properly-signed installs are unchanged. Thanks @exzanimo (#348).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "**New app icon (everywhere):** a bolder take on the Titanium & Gold mark — a thick **gold recovery ring + core** on deep navy, across iPhone, Mac and Android (and the in-app logo). Same Kineva, sharper identity.",
        ...
        Release(
            ...
            items: [
                ...
                "Bluetooth & live HR: WHOOP 5/MG keeps decoding correctly after iOS relaunches Kineva in the background, and the Lock-Screen / Dynamic-Island live heart rate now ends when the strap disconnects instead of freezing on a stale number. Thanks @vulnix0x4 (#378, #386).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed: a corrupt or mis-aligned Bluetooth frame could wedge the live data stream until you reconnected — Kineva now spots an impossible frame length and resyncs to the next real frame instead of stalling. Thanks @vulnix0x4 (#374).",
                "Fixed (iPhone): the two-way Apple Health sync was reading its OWN written-back values back in as \"Apple Health\" data — which could make your strap and Apple Health plot the same line, and skew the Apple-Health average if you also wear a watch. It now excludes Kineva's own samples on read, and a failed sync no longer reports a false \"success\". Thanks @vulnix0x4 (#375).",
            ]),
        Release(
            version: "3.0.1",
            ...
            date: "June 2026",
            items: [
                "Changed: removed the small gold dot in the centre of the Charge / Recovery rings, behind the number — at the v3 launch a few of you (rightly) said it crowded the read-out. The clean ring + number + micro-Kineva wordmark stay; the dot now lives only in the standalone logo.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New: Kineva's biggest redesign yet — \"Titanium & Gold\". A deep-navy canvas, a warm gold accent, brushed-titanium detail and a per-domain colour world (blue sleep, amber strain, teal HRV, burnt-orange stress), in Helvetica, across iPhone, Android and Mac.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (experimental): Settings now has an Export raw sensor data (CSV) button — it dumps the decoded per-sample streams Kineva already stores (heart rate, R-R, accelerometer, the motion/step counter, SpO2/PPG and events) for the last 24h as a plain CSV. It's for tinkerers: prototype your own sleep / activity / VBT algorithms on real data, no BLE coding needed. On-device only, nothing leaves your phone unless you share it. Thanks @maddognik / @alacore (#322/#276).",
            ]),
        Release(
            version: "2.17.1",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "The new look, everywhere: every screen now wears Kineva's premium dark design — scenic backdrops, glowing ring gauges and frosted per-domain cards across Sleep, Recovery, Stress, Workouts, Live, Health, Trends, Insights, Breathe, Coach and Settings, on Mac, iPhone and Android.",
                ...
                "Fixed (sleep detection): on WHOOP 5.0 a full night is no longer chopped into tiny fragments and dropped — Kineva now holds the night together from your heart rate when motion data is sparse. Thanks @umarXBT (#308).",
                ...
                "Fixed (Android Bluetooth): turning Bluetooth off — or flight mode — no longer leaves Kineva showing a phantom \"connected\" or crashing on the next buzz; it now cleanly shows disconnected and reconnects when Bluetooth returns. Thanks @pilleuspulcher-blip (#314).",
            ]),
        Release(
            version: "2.14.1",
        ...
        Release(
            ...
            items: [
                ...
                "New (iPhone): **two-way Apple Health** you can actually turn on — enable it on the Apple Health screen and your Kineva recovery, HRV, resting HR and more flow to Health (now including strap-only users), with the Apple Health screen finally populating. Thanks @vulnix0x4.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Android): a **smart wake alarm** — set a wake window and Kineva wakes you on a lighter sleep phase inside it, with a guaranteed alarm at the end of the window. The guaranteed wake is a real OS alarm that fires even if Bluetooth drops or the app is closed. Thanks @subscriptiondestroyer (#207).",
                ...
                "New: an **Effort scale toggle** — show Effort on Kineva's 0–100 axis or WHOOP's familiar 0–21 Day-Strain axis, everywhere it appears. Display-only; your stored data is unchanged. Thanks @umarXBT (#268).",
        ...
        Release(
            ...
            items: [
                ...
                "Improved: body vitals now show which source each reading came from (your WHOOP, Kineva's own computation, or Apple Health) and merge them field-by-field instead of letting one source blank the others. Thanks @khalilkm01.",
                "New (Android): tap-and-drag to inspect the Stress chart, and a cleaner Explore metric picker. Thanks @ujix.",
                "New: an optional, read-only local access package (MCP) for power users who want to query their own on-device Kineva data from local tools — opt-in, nothing leaves the device. Thanks @khalilkm01.",
        ...
        Release(
            ...
            items: [
                ...
                "New (Android): if Kineva ever crashes, the details are now saved into the strap log you share — so a crash that only happens on your device can actually be diagnosed (#33).",
                "More accurate HRV: the heart-rate variability Kineva computes from a session now discards stray, irregular beats before averaging — the same cleaning the rest of its HRV maths already does — so a noisy WHOOP 5/MG optical reading no longer comes out inflated. Thanks @frazzle28 (#262/#235).",
        ...
        Release(
            ...
            items: [
                ...
                "Fixed (Android): a workout imported from Health Connect could show no distance even when the distance was recorded — a relay app (e.g. Suunto via Health Sync) often writes the distance with timestamps slightly offset from the workout, which Kineva's exact-window match missed. It now matches with a tolerance. Thanks @pilleuspulcher-blip. (#215)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (iPhone): a 'Using Kineva on iPhone' note in Settings sets honest expectations — sideloading, re-signing, unlocking your phone after a reboot so history can sync — and shows how many days until your sideloaded build expires. The strap log you share now also carries the iPhone details (iOS version, lock state, background-refresh, low-power) that make iPhone-only issues quick to diagnose, with a one-tap Diagnostics screen to copy them.",
                ...
                "Improved (WHOOP 5/MG): the heart rate Kineva derives from the optical sensor on sleeping (sub-60 bpm) stretches no longer risks snapping to ~60 bpm from a recording artifact, while a genuine 60 bpm is preserved. Thanks @ryanbr (#194).",
            ]),
        Release(
            version: "2.8.5",
            ...
            date: "June 2026",
            items: [
                "Fixed (iPhone): importing a WHOOP or Apple Health export could silently do nothing — iOS was handing the app an iCloud file that hadn't downloaded yet. Kineva now downloads a local copy first (through the system Files picker), so imports actually go through. Thanks @adrnxq and @Chopin85. (#179)",
                "Fixed (iPhone): if a Kineva backup from another platform had been restored (e.g. an Android backup onto an iPhone), the app could get permanently stuck on “store not ready” — the imported database held the data but not the bookkeeping Kineva's database engine needs. Kineva now recovers automatically on the next launch, and declines such a backup at import time with a clear explanation. To move history across platforms, use the WHOOP-format CSV export instead. Thanks @NoahMcE. (#222)",
            ]),
        Release(
            version: "2.8.4",
            ...
            date: "June 2026",
            items: [
                "New: a clear in-app guide to how Kineva's three daily scores — Charge, Effort and Rest — are calculated, and how they differ from WHOOP's Recovery, Strain and Sleep. Tap the ⓘ on any score on the Today screen, or open it any time from Settings → About → How your scores work. New here? A one-time card points you to it.",
                "New: each score now explains how sure Kineva is of it — Solid, Building or Calibrating — and carries a one-line description of what it measures.",
            ]),
        Release(
            version: "2.8.3",
            ...
            date: "June 2026",
            items: [
                "Fixed (iOS): after importing your data, the strap could get stuck on \"store not ready\" and never sync — imported history wouldn't appear and backfill never started. On iOS the local database was sealed behind the device's data protection while the phone was locked, so a background reconnect couldn't open it (macOS and Android were never affected). Kineva now stores its database at the right protection level — readable after you first unlock since boot, still encrypted at rest — and retries automatically, so sync proceeds. Thanks @NoahMcE (#222).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Improved (battery): Kineva now backs off its history-sync polling when the strap keeps handing over nothing (off-wrist or not yet banking) instead of re-trying every 90 seconds — a manual or reconnect sync still runs instantly, and the first real record resumes normal cadence. Thanks @ryanbr (#217).",
        ...
        Release(
            ...
            items: [
                ...
                "Improved: charts and metrics now resolve the **freshest source** for each value (imported WHOOP, then Kineva-computed, then compatible Apple Health), so a screen never looks stale when newer data exists. Thanks @khalilkm01.",
                ...
                "Improved (AI Coach): when a local LLM truncates the conversation to fit its context window, Kineva now tells you, and caps the history it sends to local servers. Thanks @witchykinkajou.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed (WHOOP 4.0): some straps on firmware 41.17.x silently failed to set their clock, so they banked no history and showed no sleep or recovery. Kineva now sends both clock-command formats, so these straps clock and bank correctly. Thanks @rad182 (#120).",
                "Fixed: the strap sometimes wouldn't reconnect after an app update — Kineva now rotates the scan between WHOOP 4 and 5/MG so it finds your strap either way. Thanks @khalilkm01.",
                ...
                "Fixed (WHOOP 4.0): when a strap hands over an empty history sync, Kineva now reliably tells you to charge it to 100% and reconnect instead of silently showing nothing. Thanks @alberba (#214).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac, iOS and Android, experimental): a WHOOP 5.0/MG \"deep data\" unlock under Settings → Experimental. 5/MG straps give a fresh third-party app only live heart rate; the official app switches on the deeper streams by writing a set of feature flags. Kineva can now send that exact, documented sequence to your strap (opt-in, one button, only when worn + bonded). It writes to the strap but is reversible — it just changes which data the strap emits — and it is the same thing the official app does. Experimental: it may do nothing on your firmware yet. If you have a 5/MG, turning it on and sharing your strap log is exactly what we need to finish 5.0/MG support. iPhone/Android only (a Mac cannot write to a 5/MG). Built on the public protocol work of judes.club, Asherlc/dofek and b-nnett/goose. (#174)",
            ]),
        Release(
            version: "2.4.0",
            ...
            date: "June 2026",
            items: [
                "New (Mac, iOS and Android): a small card on the Today screen — at most once every 12 hours — asking whether Kineva is proving useful, with the honest numbers: a WHOOP membership runs $300–480 a year, Kineva is free, and 5,000+ downloads in, 7 people have donated. \"Later\" snoozes it 12 hours; \"Don't ask again\" turns it off forever. It's a card in the flow, never a pop-over, and the stats are baked in at release time — the app still never touches the network.",
            ]),
        Release(
            version: "2.3.2",
        ...
        Release(
            ...
            items: [
                ...
                "Fixed (Mac and iOS): the strap log no longer prints a stale \"layout v25/v26 … doesn't decode yet\" warning for layouts Kineva has decoded for a while. (#156, thanks @sudden-break)",
        ...
        Release(
            ...
            items: [
                ...
                "Fixed (Android): the AI Coach now sees the recovery, strain, sleep and HRV that Kineva computes on-device for live-strap users — it was only reading imported rows, so a Bluetooth-only user's Coach wrongly said it had no data. (#124)",
                "Fixed (Android): your imported step count now updates for TODAY, not just past days — Kineva refreshes today's Health Connect steps when you open the app. (#150)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Improved (Mac, iOS and Android): your strap log now records what a sync SAVED, not only what failed — a \"persisted N rows (M with motion) across K night(s)\" line on every successful offload. Kineva previously logged only failures, so a shared log couldn't show whether history was actually banking; now it can. (#150)",
                "Improved (Mac, iOS and Android): when the strap reports it has no stored history to hand over (its \"no flash cursor\" state), Kineva now names the real cause plainly — the strap's clock has lost sync and it isn't saving to flash, a charge/clock state on the strap, NOT a Kineva decode bug. The Troubleshooting and FAQ guides now lead with this, the most common reason recovery and sleep don't appear, with the fix: fully charge to 100% and reconnect. (#150)",
            ]),
        Release(
            version: "1.99",
            ...
            date: "June 2026",
            items: [
                "New (Android): the Today screen's Steps tile now shows the steps from your Apple Health / Health Connect import when the strap didn't bank an on-device count — so a WHOOP 4.0, which Kineva can't yet read steps off over Bluetooth, shows your imported steps instead of \"No Data\" (Mac and iOS already did this). Worth saying plainly: the WHOOP 4.0 does count steps in the official WHOOP app — the only gap was that Kineva couldn't surface them yet. (#150)",
            ]),
        Release(
            version: "1.98",
            ...
            date: "June 2026",
            items: [
                "Recovered (Android): the reject-archive retro-decode that landed on Mac & iOS in v1.97 now runs on **Android** as well. If your WHOOP 4.0 on Android synced \"v25\" firmware records before v1.95 — when Kineva couldn't read that layout — that sleep and recovery were saved but left dark; on update Kineva now re-runs them through the current decoder and backfills those nights. (#151)",
            ]),
        Release(
            version: "1.97",
            ...
            date: "June 2026",
            items: [
                "Recovered (Mac, iOS and Android): if your WHOOP 4.0 synced \"v25\" firmware records *before* v1.95 — when Kineva couldn't read that layout yet — those records were saved to Kineva's on-device archive but left dark, and the strap had already freed them. Kineva now re-runs that archive through the current decoder on update, so your sleep and recovery from those nights backfill. It happens once per decoder upgrade, automatically. (#151)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New: the iOS app is now a **direct download** you install with AltStore or SideStore — it signs on your own iPhone with your own free Apple ID, so there's no App Store, no developer account, and Kineva stays anonymous. You no longer need a Mac and Xcode to run it. (Two notes, stated plainly: a free Apple ID re-signs the app every 7 days — AltStore automates that — and some Apple-only integrations like Apple Health and Live Activity widgets can be limited under a free signing identity.)",
                ...
                "Fixed (Android): Health Connect import now respects partial permissions — switch off the data types you don't want Kineva to read, and it imports the rest instead of refusing the whole import. (#150)",
            ]),
        Release(
            version: "1.95",
            ...
            date: "June 2026",
            items: [
                "New (Mac and Android): some WHOOP 4.0 straps run a firmware whose offloaded history Kineva couldn't decode for motion — so sleep and recovery never built from the strap, even though live heart rate worked. Kineva now reads that firmware's motion (the accelerometer gravity vector) and per-second timestamps, which is exactly what the sleep engine needs. Once your strap banks a night, sleep staging and recovery can finally build from it. Heart rate in this layout is derived from the optical sensor rather than stored second-by-second, so this unlock is specifically the motion data. (#30)",
            ]),
        Release(
            version: "1.94",
        ...
        Release(
            version: "1.90",
            title: "Kineva now tells you when your strap isn't saving history — and how to fix it",
            date: "June 2026",
            items: [
                "Improved (Mac and Android): when a sync completes but your strap handed over only its diagnostic output and no stored history — which means its clock has lost sync and it isn't saving data to flash — Kineva now says so, with the fix (fully charge the strap to 100%, then reconnect), instead of silently reporting \"synced.\" It's the single most common reason recovery, sleep and strain stop appearing on a WHOOP 4.0, and it's now told apart from a normal caught-up sync. (#77, #91, #120)",
            ]),
        Release(
            version: "1.89",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed (Mac and Android): a workout you start yourself now estimates its calories from your heart rate — the same model Kineva uses for auto-detected workouts — instead of leaving the field blank. (#117)",
                "Fixed (Android): workouts imported from Health Connect (e.g. Garmin) now show their calories. Kineva credits each session with the active calories burned inside its time window (a Health Connect exercise record carries no energy of its own, so this stitches them together). (#117)",
            ]),
        Release(
            version: "1.82",
            ...
            date: "June 2026",
            items: [
                "Fixed (Mac and Android): Kineva no longer destroys strap history it can't yet decode. If a history chunk arrived with a bad checksum or a firmware record layout we haven't mapped, Kineva used to tell the strap \"got it\" anyway — and the strap then freed (erased) that data while the screen said \"synced\". Kineva now archives those raw records on-device before acknowledging, and if it can't save them it leaves them on the strap to retry, so an unrecognised firmware can no longer cost you your data. (#77, #91)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac and Android): add a workout by hand, and edit, re-label, or dismiss the ones Kineva auto-detects — so a misread bout or a duplicate no longer sticks around with no way to remove it. Dismissals are remembered, so a re-detected session stays hidden.",
                "New (Mac and Android): export all your data as a WHOOP-format CSV bundle (cycles, sleeps, workouts, journal) from Settings — yours to keep, and it imports straight back into Kineva.",
            ]),
        Release(
            version: "1.78",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac and Android): a one-time, plain-English terms acknowledgment on first launch — what Kineva is, that it's independent of WHOOP and that using it may breach WHOOP's Terms of Service, that it's not a medical device, and that you use it at your own risk. Standard for an independent, on-device tool — you accept once. The full terms ship in TERMS.md.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Improved (Mac and Android): a very large Apple Health export no longer fails to import because of a single malformed byte. Kineva now skips the bad spans and imports everything else, and tells you how many it skipped — so multi-year exports that errored out before should come in fine now.",
                "New (Mac): if your Bluetooth radio can't sustain WHOOP 4's full realtime stream (older Macs, OpenCore setups), Kineva now automatically falls back to a low-bandwidth standard heart-rate mode — so live HR keeps working instead of the connection looping on a drop.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Android now matches the Mac: if your WHOOP 5.0 / MG can't connect after a firmware update (a Bluetooth pairing reset), Kineva detects it and shows the forget-and-re-pair steps right in the app, instead of silently retrying. (Mac got this in 1.73.)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "If your WHOOP 5.0 / MG stopped connecting after a WHOOP firmware update, that's a Bluetooth pairing reset — not a lockout, and Kineva works fine on the new firmware. To reconnect: quit the official WHOOP app, forget the strap in your Bluetooth settings, put it in pairing mode (tap the band until the LEDs flash blue), then reconnect. On Mac, Kineva now detects this automatically and shows you these exact steps in-app instead of silently retrying. WHOOP 4.0 is unaffected.",
            ]),
        Release(
            version: "1.72",
            ...
            date: "June 2026",
            items: [
                "Fixed (Android): starting a GPS-tracked workout could crash the app on Android 12 and newer. GPS needs location permission, which Kineva never requested — and it was capped to older Android versions — so route tracking failed the instant it began. Kineva now asks for location permission right before a GPS workout and fails safe if it's unavailable: the workout still records heart rate and strain, just without a route. If you don't use GPS workouts, nothing changes. (Mac: version bump only.)",
            ]),
        Release(
            version: "1.71",
        ...
        Release(
            ...
            items: [
                ...
                "Improved (Mac and Android): sleep tiles now prefer WHOOP's own imported figures (sleep performance, consistency, need, debt) when available, falling back to Kineva's on-device estimate otherwise — and Android now imports those four figures too.",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac and Android): start and stop a workout yourself, instead of waiting for Kineva to detect one. Tap Start workout on the Live screen and you get a live card — elapsed time, heart rate, and strain building in real time; tap End and it's scored and saved to your Workouts, contributing to the day. Perfect for a session Kineva might not auto-detect, or when you just want a clean start/stop. Needs a connected strap streaming live heart rate. A community request — thanks for the nudge.",
            ]),
        Release(
            version: "1.66",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Diagnostics (Mac and Android): if a chunk of history arrives from the strap but none of it can be decoded — frames failing their checksum, an unrecognised firmware layout, or out-of-range timestamps — Kineva now says so plainly in the strap log instead of quietly moving on. Until now a sync like that looked completely healthy (\"history synced\") while the data went nowhere, which made a rare \"I wore it but got no data\" report almost impossible to diagnose. This release changes no behaviour — it just makes that case visible — so if your history isn't showing up, turning on Debug logging and sharing your strap log will now point straight at the cause. Investigating a report along these lines (#77).",
            ]),
        Release(
            version: "1.64",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac and Android, experimental): Kineva now sets the clock on a WHOOP 5.0/MG before asking for its history — and that matters more than it sounds: an un-clocked WHOOP 5 doesn't save sensor data at all, so history syncs were \"succeeding\" with nothing in them. A fellow developer's work on real 5/MG hardware found this (history went from 0 to hundreds of frames once clocked) along with several smaller protocol fixes Kineva now carries: the history request waits for the strap to acknowledge a range query first (with a retry if it stays silent), an Android 5/MG connects directly to the strap your phone already paired instead of re-scanning, fresh history is scored within seconds instead of at the next 15-minute tick, and the strap's own diagnostic messages now appear in the strap log. Also new (Android, opt-in, default OFF): \"Record 5/MG raw capture\" in Settings → Experimental writes each history sync's raw frames to a shareable file — if you have a 5/MG, sharing one capture is the single most useful thing you can do to help Kineva learn to decode 5/MG sleep, recovery and strain. With thanks to tajchert, whose hardware-validated fork drove this release.",
            ]),
        Release(
            version: "1.61",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Android, opt-in): Kineva can now write the nightly metrics it computes from your strap — resting heart rate, HRV, SpO₂ and respiratory rate — into Health Connect, so other apps can use them. Off by default; flip \"Share back to Health Connect\" in Data Sources and grant the write permissions. Only Kineva's own computed values are written (imported data is never echoed back), and re-writes update in place rather than stacking duplicates. Mac: version bump only.",
            ]),
        Release(
            version: "1.58",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Android): a home-screen widget. Today's recovery — coloured green, amber or red by the usual bands — plus live heart rate and strap battery, at a glance without opening the app. It updates from the background connection (or while the app is open), shows when it last heard from the strap, and tapping it opens Kineva. Long-press your home screen → Widgets → Kineva to add it. Honest-blank until Kineva has learned enough nights to score you. Mac: version bump only.",
            ]),
        Release(
            version: "1.56",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Mac): recovery now builds from the strap's own offloaded nights, no WHOOP export needed — the same fix Android got in v1.53. The recovery baseline previously only learned from imported history, so a Bluetooth-only Mac user never crossed the \"learn your baseline\" threshold and recovery stayed blank. Kineva now seeds the baseline from the nights it computes on-device too, so after about four nights recovery lights up on its own. Honest-blank until then; a real import still wins per day. Also: the WHOOP 5.0/MG step counter now persists on Mac (parity with Android — surfaced later, still APPROXIMATE). Android: version bump only (it already had both).",
            ]),
        Release(
            version: "1.54",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "New (Android): recovery now builds from the strap's own offloaded nights — no WHOOP export needed. Before, the recovery baseline only ever learned from imported history, so a Bluetooth-only user never crossed the \"learn your baseline\" threshold and recovery stayed blank forever. Kineva now seeds the baseline from the nights it computes on-device too, so after about four nights of wear recovery lights up on its own. It stays honestly blank until then, and a real WHOOP import still wins per day. The natural payoff of the v1.52 offload work. Thanks to a community contribution (#78). (macOS recovery-seeding parity is a follow-up; version bump only this release.)",
            ]),
        Release(
            version: "1.52",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed: the battery flashing 100% before correcting to the real value (and sometimes reverting to 100%). A WHOOP 4.0's standard Bluetooth battery characteristic is a stub that always says 100 — the real charge comes from the proprietary battery command — and Kineva read both. It now uses only the real source per strap model. Mac and Android (#77).",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed (Android): on phones whose Bluetooth stack gets congested (a Pixel 7 on Android 16 logged dozens of \"busy\" command retries and a few dropped commands in 10 minutes), Kineva now retries a busy command more times with an escalating wait so nothing hard-drops, and re-subscribes the live channels at most once per quiet spell instead of every 30 seconds — that repeated re-subscribing was flooding the link with writes that collide with commands on phones that only allow one Bluetooth operation at a time. Steadier live HR and fewer dropped commands as a result. macOS: version bump only (it uses CoreBluetooth's own queue and isn't affected).",
            ]),
        Release(
            version: "1.49",
            ...
            date: "June 2026",
            items: [
                "Fixed: Spanish WHOOP CSV exports now import. A Spanish export translates both the column headers (Puntuación de recuperación, Variabilidad de la frecuencia cardíaca, and so on) and some filenames (sueño.csv, entrenamientos.csv), so it used to match nothing and reported \"Imported 0 items.\" Kineva now maps the Spanish columns to their canonical fields and recognises the Spanish filenames, so recovery, strain, sleep, HRV and the rest come through correctly. Mac and Android. Thanks to a reporter who supplied a real export's headers (#76) — the same way German was added.",
            ]),
        Release(
            version: "1.48",
            ...
            date: "June 2026",
            items: [
                "Fixed (Android): on some phones — especially newer ones on Android 13+, and worst on Android 16 — Kineva could silently drop a Bluetooth command when the phone's Bluetooth stack was momentarily busy, instead of retrying it. The dropped command was often the one that starts live heart rate, sets the strap clock, or acknowledges a chunk of history — so live HR sometimes never started and overnight data didn't come through, even though the strap and pairing were fine. Kineva now retries a rejected command and paces the writes so the stack keeps up. Thanks to a detailed strap log from a Pixel 7 on Android 16 (#77). (macOS: version bump only — it uses CoreBluetooth's own write queue and was never affected.)",
            ]),
        Release(
            version: "1.47",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Improved (Mac): live heart rate on a WHOOP 5.0/MG streams even before the strap is fully paired — but buzz, alarms, double-tap and full history sync all need that real pairing. Kineva now keeps the \"free the strap from the WHOOP app\" guidance visible (in clearer wording) whenever the strap isn't fully paired, so it's obvious what to do to unlock the rest. Thanks to a 5.0/MG report (#69).",
            ]),
        Release(
            version: "1.44",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed: the Smart alarm wake time didn't always reach the strap. If you changed the time while the strap wasn't actively connected, the new time silently never transmitted — so the strap kept its old time (you set 07:15, but it still buzzed at 07:00). Kineva now re-sends the alarm time every time the strap reconnects, so the time you set is the time that fires. Mac and Android.",
            ]),
        Release(
            version: "1.32",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Decoding progress (WHOOP 5.0): Kineva now reads skin temperature, motion/activity and wrist-contact from your 5.0's stored history — each verified against real data (e.g. ~30.6 °C on the wrist, dropping to room temperature off it) and only stored when it's physically sensible. These are building blocks toward on-device 5.0 sleep and recovery; nothing changes on screen yet.",
                "Fixed (Mac): corrected which byte Kineva reads the 5.0's optical-pulse channel from — a community reverse-engineering report, cross-checked against our own captured frames, showed it was a counter byte, not the channel. The pulse waveform itself was always decoded correctly; this only affects the channel label.",
            ]),
        Release(
            version: "1.20",
        ...
        Release(
            ...
            items: [
                ...
                "Decoding progress (WHOOP 5.0): Kineva now reads the strap's raw optical pulse (PPG) waveform from its stored history — a 24 Hz trace verified against your own heart rate, with no external reference. Nothing changes on screen yet; it's a building block toward 5.0 recovery and strain.",
            ]),
        Release(
            version: "1.18",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "WHOOP 5.0/MG on Mac: the secure pairing now completes and live heart rate comes through. Kineva waits for the strap to bond before subscribing to its data channels — subscribing too early was the silent failure — then asks it to start streaming with the right framing. If the strap won't bond on first connect, Kineva now tells you to close the official WHOOP app and put the strap in pairing mode (blue LEDs flashing), which is what lets it pair. Still experimental on 5/MG; built from a 5/MG owner's verified flow. (Android 5/MG bonding landed in v1.10; WHOOP 4.0 is untouched.)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Fixed live heart rate freezing on a stale number mid-session. Kineva now keeps the strap's realtime stream re-armed and, if the link goes quiet, quietly reconnects on its own — no more disconnect-and-reconnect by hand to un-stick it. (Android now matches how the Mac app already behaved.)",
        ...
        Release(
            ...
            date: "June 2026",
            items: [
                "Kineva now keeps your strap connected when the app is closed. On Android it shows a quiet ongoing notification and keeps streaming your heart rate; on Mac, just close the window and Kineva keeps running from the menu bar.",
                "New “Keep connected in the background” toggle in Settings → Strap (on by default). Turn it off and Kineva disconnects whenever you close the app.",
    ...
    static let expectations: [Expectation] = [
        Expectation(
            ...
            body: "Kineva is a personal, open project — not the WHOOP app, and not affiliated with WHOOP. It reads a strap you own, on your own device. Treat it as a capable work-in-progress rather than a finished product."),
        Expectation(
            icon: "checkmark.seal",
            title: "WHOOP 4.0 is the supported path",
            body: "WHOOP 4.0 is tested and works end to end. WHOOP 5.0/MG is newer: live heart rate works today, but deeper metrics (recovery, strain, sleep) for 5/MG are still being figured out. Kineva always tells you what's live versus still building."),
        Expectation(
            icon: "hourglass",
            title: "Your scores build over a few nights",
            body: "Live heart rate is instant. Recovery, strain and sleep sharpen as Kineva learns your baseline over your first nights of wear. Want your history now? Import your WHOOP export in Data Sources and it backfills in about a minute."),
        Expectation(
            icon: "lock.shield",
            title: "Everything stays on your device",
            body: "No account, no cloud, no sync. Kineva talks only to your strap and keeps everything local. Your data is yours alone."),
    ]
}

...
/// captioned with its in-range state. Re-skinned to the locked Kineva component
        ...
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            NoopCard(tint: StrandPalette.chargeColor) {
                VStack(alignment: .leading, spacing: NoopMetrics.cardInnerSpacing) {
                    ...
                    // Route the manual offload kick through the unified Kineva button system so the
...
/// One README "zone / stage bar": a label + qualitative word on top, the Kineva signature segmented
...
private struct VitalsSection: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            Text("Once Kineva has 14 nights of history, in-range compares each vital to your own baseline (approximate — not medical advice); until then, typical adult ranges apply.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
        }

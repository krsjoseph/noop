...
struct AddDeviceWizard: View {
    ...
    enum DeviceType: Identifiable, Hashable {
        ...
        case oura          // Oura ring (factory-reset-and-adopt: Kineva installs its own key, becomes owner)
        var id: Self { self }
    ...
    /// before the wizard will scan, because adoption installs Kineva's key and the Oura app stops working
    ...
    var body: some View {
        ...
        .alert("Take over this ring?", isPresented: $ouraConfirmAdopt) {
            ...
            Text("Kineva will install its own key on the ring and become its owner. The Oura app will no longer control this ring. This is intended and it cannot be undone from Kineva.")
        }
    ...
    private func prepInstructions(_ t: DeviceType) -> [String] {
        ...
        case .oura:
            // The factory-reset-and-adopt checklist, shown only AFTER the irreversible-consent gate. Kineva
            ...
            return [
                "Open the official Oura app and remove this ring (Oura calls it \"factory reset\" or \"unpair and reset\"). This wipes the ring's owner so Kineva can take it over.",
    ...
    @ViewBuilder private var ouraConsentGateFace: some View {
        ...
        StrandCard(padding: 16) {
            VStack(alignment: .leading, spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ouraBullet("Your ring talks to Kineva only, fully offline, no Oura account.")
                    ...
                    ouraBullet("Kineva's own Charge, Effort and Rest, computed on your device from published methods.")
                }
                Divider().overlay(StrandPalette.hairline)
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ouraBullet("Oura's own Readiness and Sleep scores. Kineva does not copy them. It computes its own.")
        ...
        Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                Image(systemName: ouraConsented ? "checkmark.square.fill" : "square")
                    ...
                    .accessibilityHidden(true)
                Text("I understand this disconnects the ring from Oura and that Kineva cannot undo it for me. To go back to Oura I would factory-reset the ring again and set it up in the Oura app.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textPrimary)
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("I understand this disconnects the ring from Oura and that Kineva cannot undo it for me.")
    ...
    @ViewBuilder private var ouraAdvancedKeyFace: some View {
        ...
        Text("Kineva stores this key only on this device, in the same place it stores your paired bands.")
            .font(StrandFont.footnote)
            .foregroundStyle(StrandPalette.textTertiary)
            .fixedSize(horizontal: false, vertical: true)
    ...
    @ViewBuilder private var ouraConfirmFace: some View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if ouraAdvancedKeyMode {
                ...
                Text("Both Kineva and the Oura app can use a ring you own by key, but only one can hold the Bluetooth link at a time.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            } else {
    ...
    /// The Adopting face: an honest "Installing Kineva's key" progress card shown ONLY while a real key install
    ...
    @ViewBuilder private var ouraAdoptingFace: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Installing Kineva's key and confirming the ring answers only to Kineva. Keep the ring close and do not open the Oura app.")
                .font(StrandFont.subhead)
                .foregroundStyle(StrandPalette.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
        }
    ...
    @ViewBuilder private var ouraFailedFace: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("We could not take over this ring.")
                .font(StrandFont.headline)
                .foregroundStyle(StrandPalette.textPrimary)
            Text(model.ouraNeedsPairing ?? "The most common cause is the ring was not fully reset in the Oura app, or the Oura app is still running. Reset the ring again, force-quit Oura, then try once more. If it keeps failing, your ring may be a generation Kineva cannot adopt yet. The ring is not bricked: re-pair it in the Oura app to recover it. You can still use file import.")
                .font(StrandFont.subhead)
                .foregroundStyle(StrandPalette.textSecondary)
                .fixedSize(horizontal: false, vertical: true)
            HStack(spacing: 10) {
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Beta. Read this first.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.statusWarning)
                Text("Local Oura support is new and we cannot test every ring here. It may not connect on your ring, and it can change between updates. Kineva never makes up a number. If something does not work, it will tell you plainly.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.statusWarning)
                    .fixedSize(horizontal: false, vertical: true)
            }
    ...
    private var confirmBrand: String {
        ...
        // Oura confirms with the detected generation name + a Beta marker so the user sees what Kineva
    ...
    /// (OuraRingGen.from(model:) recovers it), and the capability set is gen-filtered. Kineva computes its own
    ...
    /// stored here: the live install persists Kineva's freshly-generated key only on an OK `0x25` ack.

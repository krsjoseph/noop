...
//
// "Your own logbook." Kineva gives you a private place to KEEP the numbers you already
// get from your doctor or pharmacy — bloods, blood pressure, body measurements — and
// SEE them next to your wearable signals, entirely on this device. Kineva never tests
...
// asserts a clinical judgement — never "abnormal/high/low/normal" as Kineva's own
...
struct LabBookView: View {
    ...
    private var headerCard: some View {
        NoopCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("It's a notebook, not a lab. Kineva lines up the numbers you enter — it doesn't test, read, or judge them. Not medical advice.")
    ...
    /// NEUTRAL (textTertiary): a marker rising or falling is not "good" or "bad" — Kineva never imputes a
    ...
    private var disclaimerNote: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Lab Book is a private notebook, not a medical service. Kineva stores and lines up the numbers you enter — it doesn't test, read, diagnose, or advise. Your records never leave \(Platform.deviceNounPhrase); there's no account or cloud, so it isn't \"HIPAA-covered.\" Always rely on your doctor or pharmacist to interpret results.")
    ...
    var body: some View {
        ScreenScaffold(title: LocalizedStringKey(displayName),
            ...
            VStack(alignment: .leading, spacing: NoopMetrics.sectionGap) {
                ...
                historySection
                Text("These are your own numbers shown back to you. Kineva doesn't decide whether any value is normal, high or low.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        VStack(alignment: .leading, spacing: NoopMetrics.gap) {
            ...
            NoopCard {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if signal == nil {
                        Text("Pick a wearable signal (resting HR, HRV, sleep, Charge, weight…) to line it up against this marker. Kineva averages the signal over the \(window.phrase) before each reading.")
                            .font(StrandFont.subhead)
                            .foregroundStyle(StrandPalette.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    } else {
    ...
    private var resultBlock: some View {
        ...
        if computing {
            ...
            Text(n == 0
                 ...
                 : "\(n) reading\(n == 1 ? "" : "s") line up so far — not enough to read a trend yet (Kineva waits for \(LabBookSignals.floor)).")
                .font(StrandFont.subhead)
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
...
private struct LabBookDisclaimerView: View {
    ...
    var body: some View {
        ScreenScaffold(title: "About Lab Book", subtitle: "A private notebook, not a medical service.") {
            VStack(alignment: .leading, spacing: NoopMetrics.gap) {
                bullet("Kineva stores and lines up the numbers you enter yourself. It does not test you, read your results, give medical advice, or diagnose anything.")
                ...
                bullet("Kineva never decides whether a value is \"normal,\" \"high,\" or \"low.\" Any reference range shown is exactly what you typed from your own report.")
                bullet("Your records never leave \(Platform.deviceNounPhrase). There's no account, no cloud, no Kineva server. Because Kineva is an independent app you run yourself — not a healthcare provider — it isn't \"HIPAA-covered,\" and that protection doesn't apply here; the safety comes from the data being local-only and yours.")

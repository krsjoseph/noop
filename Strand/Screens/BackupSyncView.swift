...
struct BackupSyncView: View {
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .center, spacing: 16) {
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("Backs up to your folder about once a day and keeps the latest \(FolderBackup.keepCount). On this platform it runs when you next open Kineva.")
                            .font(StrandFont.footnote).foregroundStyle(StrandPalette.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                    Spacer(minLength: 0)
    ...
    private func openRestorePicker() {
        ...
        if snapshots.isEmpty {
            ...
            alertMessage = "There are no Kineva backups in your folder yet. Use Back up now first."
            showAlert = true
        } else {
    ...
    private func runRestore(_ snap: FolderBackup.Snapshot) {
        ...
        Task {
            ...
            await MainActor.run {
                ...
                case .imported:
                    alertTitle = "Restored"
                    alertMessage = "Fully quit and reopen Kineva to load it."
                case .failure(let m):

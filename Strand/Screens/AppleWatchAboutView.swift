...
// Watch-as-a-device project). Kineva can run off only an Apple Watch (the phone computes our
...
struct AppleWatchAboutView: View {
    ...
    private let metrics: [WatchMetric] = [
        ...
        WatchMetric(icon: "heart.fill", metric: "Recovery / Charge",
                    ...
                    detail: "Led by your heart-rate variability versus your own baseline. The watch samples HRV rather than streaming it, so this needs about a week of nights to calibrate. Until then Kineva shows \u{201C}needs more data\u{201D}, never a guessed number."),
        ...
        WatchMetric(icon: "drop.degreesign", metric: "Blood oxygen (SpO₂)",
                    ...
                    detail: "Trend only where supported, and Apple removed the SpO₂ sensor from the newest US units, so on those it simply isn't there. Kineva shows nothing rather than a fake reading."),
    ]

    var body: some View {
        ScreenScaffold(title: "About Apple Watch data",
                       subtitle: "What your watch is great at, where it's lighter than a chest strap, and how sure Kineva is.",
                       lazy: true) {
    ...
    private var introCard: some View {
        NoopCard(tint: StrandPalette.accent) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("Kineva can run off only an Apple Watch, no chest strap needed. The watch is the sensor; Kineva does the thinking on your phone, computing Charge, Rest, Effort and your Fitness Age from your Health data, all on-device.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                Text("The honest catch: a watch isn't a chest strap. It's brilliant at sleep, steps, workouts and fitness, and lighter on the dense heart-rate-variability a strap measures all night. So recovery takes about a week to calibrate, and a couple of metrics depend on your watch model. Kineva is upfront about all of it. Every watch-derived number carries a confidence, and where the watch can't be honest, Kineva shows nothing instead of a made-up figure.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        NoopCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text("A couple of metrics depend on which Apple Watch you wear. Wrist temperature, which feeds skin temp, arrived with Series 8, so older watches don't report it. Blood oxygen is the bigger one: Apple removed the SpO₂ sensor from the newest US units over a patent dispute, so those simply don't measure it.")
                    ...
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
                Text("Where a sensor isn't on your watch, Kineva reads \u{201C}not available\u{201D} for that metric, never a zero, never an invented number. Everything else keeps working.")
                    .font(StrandFont.subhead)
                    .foregroundStyle(StrandPalette.textSecondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
        ...
        NoopCard(tint: StrandPalette.accent) {
            VStack(alignment: .leading, spacing: 12) {
                Text("Ready to connect your watch?")
                    .font(StrandFont.headline)
                    .foregroundStyle(StrandPalette.textPrimary)
                Text("Kineva reads your Apple Watch data through Apple Health, on your phone, nothing leaves the device. You choose exactly what to share.")
                    .font(StrandFont.footnote)
                    .foregroundStyle(StrandPalette.textTertiary)
                    .fixedSize(horizontal: false, vertical: true)
                Button(action: start) {
    ...
    private var footerNote: some View {
        Text("These are independent estimates computed on your device from your Apple Health data, not medical advice. Confidence labels are honest about how much Kineva knows so far.")
            .font(StrandFont.footnote)
            .foregroundStyle(StrandPalette.textTertiary)
            .fixedSize(horizontal: false, vertical: true)
            .frame(maxWidth: .infinity, alignment: .leading)

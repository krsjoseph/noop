...
struct TrendsReportPage: View {
    ...
    private var header: some View {
        ...
        // contrast carries the edge; the blue Kineva wordmark is the only accent.
        ZStack(alignment: .leading) {
            ...
            VStack(alignment: .leading, spacing: NoopMetrics.space1) {
                HStack(alignment: .firstTextBaseline) {
                    BrandMark(size: 22)
                    Text("Kineva").font(StrandFont.overline).tracking(StrandFont.overlineTracking)
    ...
    private var footer: some View {
        VStack(alignment: .leading, spacing: NoopMetrics.space1) {
            ...
            // are directly measured vs. Kineva's own derived scores. HRV / Resting HR come off the strap;
            ...
            Text("How to read this: HRV, Resting HR, Sleep duration, Respiratory rate and Skin temperature "
                ...
                + "Stress are Kineva's own on-device scores, not clinical measures — Recovery is a daily readiness "
                ...
                .foregroundStyle(StrandPalette.textTertiary)
                .fixedSize(horizontal: false, vertical: true)
            Text("Generated by Kineva on \(generatedOn) · all on-device, no account, no cloud.")
                .font(StrandFont.footnote)
                .foregroundStyle(StrandPalette.textTertiary)
...
struct TrendsReportSheet: View {
    ...
    private func export(_ report: RangeReport) {
        ...
        exporting = true
        let name = "Kineva-trends-\(report.start)_to_\(report.end).pdf"

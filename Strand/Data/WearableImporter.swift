...
/// any sleep score) are stored under REFERENCE metric keys only — never as Kineva's Charge/Effort/Rest.
/// Kineva recomputes its own scores downstream from the imported raw RHR / HRV / sleep, exactly as it
...
enum WearableImporter {
    ...
    static func importExport(url: URL, into store: WhoopStore,
        ...
        // Day rollups → DailyMetric. HRV/recovery/respiration that the export lacks stay nil; Kineva
        ...
        // browseable but never mistaken for a Kineva score.
        ...
        for d in result.days {
            ...
            // Reference-only: the brand's own scores, never a Kineva Charge/Effort/Rest input.

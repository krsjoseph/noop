...
///
/// Kineva keeps everything in one SQLite file (`<AppSupport>/OpenWhoop/whoop.sqlite`, plus the
...
enum DataBackup {
    ...
    static func runExport(checkpoint: @escaping () async -> Bool) async -> BackupResult {
        ...
        catch { return .failure("Couldn't locate the Kineva database. \(error.localizedDescription)") }

        let dbURL = URL(fileURLWithPath: dbPath)
        guard FileManager.default.fileExists(atPath: dbPath) else {
            return .failure("There's no Kineva data to export yet. Import or record some first.")
        }
        ...
        panel.title = "Export Kineva backup"
        panel.prompt = "Export"
    ...
    static func runImport() async -> BackupResult {
        ...
        panel.title = "Import Kineva backup"
        panel.prompt = "Import"
    ...
    static func restore(from pickedSource: URL, toDatabaseAt dbPath: String) -> BackupResult {
        ...
        guard isSQLiteFile(at: source) else {
            return .failure("That file isn't a Kineva backup — it doesn't look like a SQLite database.")
        }
        ...
        // Kineva-Mac/iOS backup always carries `grdb_migrations`; reject everything else that holds data.
        ...
        if origin == .android || (origin == .unknown && holdsData) {
            return .failure("This isn't a Kineva backup from this app — it's missing the migration bookkeeping a Kineva backup carries (it looks like an Android backup or another app's database), and restoring it would strand your store. To move your history across platforms, export the WHOOP-format CSV on the other device (Settings → Export data) and import that here, or import your original WHOOP / Apple Health export.")
        }
    ...
    /// "Kineva-backup-2026-06-07.noopbak"
    private static func defaultBackupName() -> String {
        let f = DateFormatter()
        f.locale = Locale(identifier: "en_US_POSIX")
        f.dateFormat = "yyyy-MM-dd"
        return "Kineva-backup-\(f.string(from: Date())).noopbak"
    }
    ...
    /// Which platform produced a Kineva backup, judged by its migrator's bookkeeping table.

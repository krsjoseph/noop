...
//
// Kineva stores EVERYTHING in SI (km, kg, cm, °C) — the importers normalise on the way in, so this is a
...
//
// Persisted via @AppStorage (UserDefaults), the same mechanism every other macOS Kineva preference uses.
...
/// How the Effort score is displayed (#268). Kineva's Effort is stored 0–100 (StrainScorer.maxStrain = 100);
/// people coming from WHOOP often think in its 0–21 Day Strain axis, so this purely cosmetic toggle lets
/// the SAME stored value be shown on either scale. Default is Kineva's own 0–100 — the data never changes.
enum EffortScale: String, CaseIterable, Identifiable {
    /// Kineva's native 0–100 axis (the stored value, one decimal).
    case hundred
...
enum UnitPrefs {
    ...
    /// resolves to `.hundred` (Kineva's native axis). Mirrored on Android by NoopPrefs("effort.scale").
    ...
    /// Resolve the stored Effort-scale raw value, defaulting to Kineva's native 0–100 axis.
...
enum UnitFormatter {
    ...
    /// Kineva stores Effort 0–100 (StrainScorer.maxStrain = 100). WHOOP's Day Strain axis is 0–21, and

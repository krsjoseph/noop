# Kineva — On-Device Data Model

Kineva is a standalone, fully offline companion app for WHOOP straps (4.0 and 5.0). It talks to
...
> device and the data it has already recorded. Kineva is **not affiliated with, endorsed by, or
...
decoded-stream tables. It dates from a since-removed server-upload feature. **Kineva is fully
...
bytes survive even for frames Kineva can't yet fully decode. Whereas the decoded streams are durable,
...
Kineva's strap interoperability is built on community reverse-engineering work, which it credits and
builds upon:
...
> **Reminder.** Kineva is not affiliated with WHOOP and is not a medical device. All stored data is

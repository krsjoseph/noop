...
> hardware this software interoperates with. Kineva reads **your own data** off **your own device**;
...
2. **Interoperability, not impersonation.** Kineva talks to a strap the user already owns. It does not
...
(`KinevaiOS` / `KinevaiOSWidgets`) — no App Store/TestFlight, to keep the project anonymous (see
...
The scheme is `Strand`; the built product is `Kineva.app` (`project.yml` sets `PRODUCT_NAME: Kineva`,
bundle id `com.aretetechnologies.kineva`). The app is **sandboxed** with the Bluetooth and
...
- **Licensing.** By opening a pull request you agree your contribution is licensed under the same
  [PolyForm Noncommercial License 1.0.0](../LICENSE) as the rest of Kineva. Forks and personal,
...
Kineva follows [Semantic Versioning](https://semver.org) — `MAJOR.MINOR.PATCH`:
...
Kineva's logic already lives in cross-platform packages, so most platform work is app-layer wiring
...
- **iOS (build-from-source target on `main`).** iOS was folded into `main` in v1.94 as a first-class
  build-from-source target — the `KinevaiOS` and `KinevaiOSWidgets` schemes (app target plus widgets, a
  ...
  identity), which is at odds with Kineva staying anonymous, so there are no pre-built downloads. Every
...
*Kineva is an independent, unofficial, non-commercial interoperability project, not affiliated with,

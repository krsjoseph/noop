# Kineva - Oura Ring BLE Protocol Specification (Clean-Room)

**Status:** Internal decoder foundation, v0.1 (2026-06-29)
**Scope:** Oura Ring Gen 3 (Horizon), Gen 4, Gen 5. Foundation for Kineva's own Swift (`StrandiOSShared` / `Strand`) and Kotlin decoders.
**Authorship:** This is Kineva's own original specification. Every protocol *fact* (UUID, opcode, byte layout, tag value) is cited to a reverse-engineering reference read for facts only. No source code was copied from any RE repo. Kineva decodes raw signals plus the ring's own HRV/sleep tags and runs Kineva's own scoring; Kineva never touches Oura's encrypted PyTorch scores.
...
> Implementation note for Kineva: subscribe to `...0003` notifications, then write commands to `...0002` with `.withoutResponse`. Do **not** assume write-with-response.
...
- **Kineva rule:** immediately after subscribing to `…0003`, request ATT MTU = **247** (Gen 4/5) or **203** (Gen 3). On iOS/CoreBluetooth the MTU is auto-negotiated; read `maximumWriteValueLength` and `CBPeripheral.maximumWriteValueLength(for: .withoutResponse)` and clamp writes. On Android, call `requestMtu(247)` before the first command.
...
> [ringverse]'s EVENTS.md states the header as `type:1, len:1, ringTimestamp:u32 LE` (i.e. it treats the 4 counter/session bytes as one u32). These are two equivalent views of the same 4 bytes; Kineva stores `ringTimestamp` as a single u32 LE and derives counter/session only if a generation needs the split. [open_ring][ringverse]
...
There is no application-level fragmentation header beyond the TLV `len`. A record never spans two notifications in the verified corpus; each notification contains whole frames/records. Kineva's parser must still be defensive: buffer partial trailing bytes across notifications and only emit complete `2+len` records.
...
Used once after a factory reset to provision Kineva's own 16-byte AES key into the ring: [ringverse][open_oura-r3]
```
phone → ring:  24 10 <16-byte key>          (SetAuthKey; 0x10 = 16-byte len)
ring  → phone: 25 01 00                       (status 0x00 = OK)
```
Kineva stores its 16-byte key locally (Keychain on iOS, EncryptedSharedPreferences/Keystore on Android). This key is required for every subsequent session's challenge.
...
**Kineva uses byte[2] of the API triplet (generation marker via product info) to branch decoder behaviour - see §7.**
...
### 5.3 Canonical fetch loop (Kineva)
...
> Behaviour caveat: [open_oura-r3] reports that on its Ring-3 unit, realtime `0x06`-based enabling ACK'd but emitted no stream within 60–90 s, whereas the `0x2F`/feature-`0x02` path above produced ~1 Hz IBI. **Kineva must use the feature-`0x02` (`0x2F`) path, not `0x06`,** and treat absence of `0x28` pushes within ~10 s as "not streaming → retry/reseat."

### 5.7 DHR auto-revert
Daytime-HR auto-reverts after ~20 s; Kineva re-engages every ~15 s while a live session is open. [open_ring]
...
bits 14–15 : qual_b
```
**Kineva filter:** accept sample only if `qual_a ≤ 1 && qual_b == 0`. [open_ring]
...
- **Kineva note:** `0x5D` is the ring's own RMSSD-derived HRV tag; Kineva also reconstructs RMSSD/SDNN itself from the IBI streams (`0x60`/`0x80`) for our own scoring (we do not consume Oura's encrypted scores). [open_ring][ringverse]

### 6.10 Battery - `0x0D` response (8 B body)
- Layout: `percent, charging_progress, recommended_flag, 3 unknown bytes`. [open_oura-r3]
- Voltage as **uint16 LE at body offset [4..6]** per [open_ring]. **CONFLICT:** [open_oura-r3] reads percent at body[0]; [open_ring] reads voltage at [4]. **Kineva rule:** read percent at body[0]; derive a voltage-based estimate from [4..6] only as a fallback, fixture-validated per generation.
...
| `0x05` | Bundling | - |
| `0x06` | Encrypted API | (Oura's encrypted channel - Kineva does NOT use) |
| `0x07` | Tap-to-tag | - |
...
### 7.3 Kineva decoder build guidance
...
4. **HRV/sleep:** consume the ring's `0x5D` (HRV) and `0x4E/0x5A` (2-bit phase codes) tags AND independently reconstruct from raw IBI/PPG for Kineva's own scoring. Never read Oura feature `0x06` (encrypted API).

...
> hardware this software interoperates with. Kineva contains no WHOOP code, firmware, or assets. All
...
`android/`, and **iOS ships as a build-from-source target (`KinevaiOS`)** folded into main in v1.94 —
...
├── Strand/                     # macOS SwiftUI app target (product name: Kineva)
...
├── StrandiOS/                  # iOS SwiftUI app shell (product name: KinevaiOS)
...
- The Xcode **scheme is `Strand`** (the target is still named `Strand` internally), but the built
  product is **`Kineva.app`** — `project.yml` sets `PRODUCT_NAME: Kineva` and bundle id
  `com.aretetechnologies.kineva`, with display name `Kineva`.
...
### 3. The ad-hoc-signed `Kineva.app`
...
The product lands at `build/Build/Products/Debug/Kineva.app`. You can confirm it is ad-hoc signed:

```bash
codesign -dvv build/Build/Products/Debug/Kineva.app
# Signature=adhoc
# Identifier=com.aretetechnologies.kineva
# CodeDirectory ... flags=0x2(adhoc)
# TeamIdentifier=not set
```

Copy the bundle wherever you like (e.g. `~/Desktop/Kineva.app`) and double-click to launch. On first
...
Kineva connects over CoreBluetooth (`Strand/BLE/BLEManager.swift`). The central manager is created
on the main queue:
...
```
~/Library/Containers/com.aretetechnologies.kineva/Data/.../whoop.sqlite
```
...
at odds with Kineva staying anonymous, so the only way to run it is to build it yourself in Xcode.
...
The `KinevaiOS` app target (plus the `KinevaiOSWidgets` WidgetKit / Live Activity extension) already
...
Or open the generated project and run the `KinevaiOS` scheme from Xcode:
...
- The `KinevaiOS` and `KinevaiOSWidgets` targets deploy to **iOS 17.0**. (The shared packages still
...
(`Kineva-full.apk`) is published in [Releases](https://github.com/NoopApp/noop/releases). A sample-data **demo** flavour still

# Kineva — Cross-Platform Swift Library Reference
...
> the hardware these packages interoperate with. Kineva contains no WHOOP
> proprietary code, firmware, or assets and works only with the user's own
> device and data. **Kineva is not a medical device.** Every derived metric (HR,
...
```swift
// Package.swift  (no internal Kineva deps — standalone)
dependencies: [

# Kineva — System Architecture

Kineva is a standalone, fully **offline** companion app for WHOOP straps (4.0 and 5.0). It talks
...
> **Not affiliated with WHOOP.** Kineva is an independent, interoperability project built on
...
```
                          ┌─────────────────────────────────────────────────────────┐
   WHOOP strap (4.0/5.0)  │                     Kineva (on-device)                     │
   ────────────────────   │                                                          │
...
The strap holds a ~14-day on-device biometric store. Kineva re-offloads it the way the official client
...
- **Stuck-strap watchdog:** after each offload, `StuckStrapDetector` compares the strap's newest
  record (`GET_DATA_RANGE`) against Kineva's data frontier (`latestHRSampleTs`). Strap-ahead **and**
...
3. **Resumable safe-trim.** The strap forgets historical data only after Kineva has it durably and has
...
4. **Pure cores, thin shell.** `WhoopProtocol`, `WhoopStore`, `StrandAnalytics`, and `StrandImport`
   ...
   surface.
5. **Interoperability, not impersonation.** Kineva reads your strap and your exports for your own use.
...
Kineva's BLE protocol work builds on community reverse-engineering of the WHOOP straps:

...
How Kineva talks to a WHOOP strap directly over Bluetooth Low Energy — no WHOOP cloud and no account.
...
> **Interoperability, not impersonation.** Kineva is a companion app for a strap *you own*. It reads the
...
macOS app and the `KinevaiOS` iOS target (`project.yml`) — so this CoreBluetooth layer is shared across
...
which made it the reliable baseline while the custom channels were being mapped. Kineva still treats
...
bonding — there is no PIN, no pairing UI. Kineva performs this with a benign `GET_BATTERY_LEVEL`
...
Once bonded, Kineva runs a WHOOP-faithful lifecycle exactly once
...
**How Kineva's Apple app (macOS/iOS) triggers it (v1.5):** it writes `CLIENT_HELLO` to `fd4b0002` *with response*.
...
### Why Kineva disables it on connect
...
stream off, Kineva's primary metric source becomes the **historical offload** (next section).
...
Each `METADATA` chunk carries a **`trim_cursor`** (u32 at frame offset 17). Kineva persists the decoded +
...
Kineva uses **`patternId = 2`** — the characteristic graduated "alarm" buzz, observed as the one the
official app fires, for interoperability (`testAlarmBuzz`, `AppModel.buzz`). `numLoops` sets the
length; `STOP_HAPTICS` (122) cancels an in-progress pattern. All notification patterns in Kineva map to
...
Kineva is closed (event `STRAP_DRIVEN_ALARM_EXECUTED`=57); always `SET_CLOCK` first so the RTC is
UTC-correct.
...
Kineva's "notify" preset → effects 47, 152  →  body = [01 2F 98 00 00 00 00 00 00 00 00 00]  (12 bytes)
```
...
WHOOP 4.0 strap exposes the following sensors and actuators. Kineva only consumes what the device already
measures:
...
**Sensors the strap does NOT have** (and that Kineva therefore never fabricates): **no microphone, no
...
Kineva interoperates with a WHOOP strap you own by: scanning for its hidden custom GATT service,

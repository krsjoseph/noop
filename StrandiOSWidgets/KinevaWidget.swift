...
struct KinevaEntry: TimelineEntry {
    let date: Date
    let snapshot: WidgetSnapshot
}

struct KinevaProvider: TimelineProvider {
    func placeholder(in context: Context) -> KinevaEntry {
        KinevaEntry(date: Date(), snapshot: .placeholder)
    }

    func getSnapshot(in context: Context, completion: @escaping (KinevaEntry) -> Void) {
        completion(KinevaEntry(date: Date(), snapshot: WidgetSnapshot.load() ?? .placeholder))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<KinevaEntry>) -> Void) {
        ...
        completion(Timeline(entries: [KinevaEntry(date: Date(), snapshot: snap)], policy: .after(next)))
    }
}

/// The glanceable widget — the iOS analogue of the macOS menu-bar extra. Recovery, live/last HR,
/// and strap battery.
struct KinevaWidgetView: View {
    @Environment(\.widgetFamily) private var family
    let entry: KinevaEntry
    ...
    private var inlineText: String {
        ...
        return parts.isEmpty ? "Kineva" : parts.joined(separator: " · ")
    }
    ...
    private var home: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Text("Kineva").font(.system(size: 13, weight: .bold))
...
struct KinevaWidget: Widget {
    let kind = "KinevaWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: KinevaProvider()) { entry in
            if #available(iOS 17.0, *) {
                KinevaWidgetView(entry: entry)
